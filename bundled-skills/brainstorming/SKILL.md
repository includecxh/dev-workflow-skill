---
name: brainstorming
description: "Use when starting a new project or new feature that needs design clarification. Not needed for bug fixes, small modifications, or when CLAUDE.md Grill Me rules already cover the scope."
---
...
The orchestrator then runs Phase 2 Lite (confirming only affected items, marking others N/A). Do NOT run the Phase 2 checklist yourself.

## Checklist（🟡🔴 标准模式必须完成以下所有项；🟢 Lite 模式见上方精简流程，跳过此 Checklist）

You MUST create a task for each of these items and complete them in order (🟡🔴 only):
...
9. **Hand back to orchestrator** — design approved; orchestrator drives Phase 2 → Phase 3+4.
...
**Brainstorming's scope ends here**: produce an approved design (with spec doc in standard mode). Don't invoke writing-plans or execute Phase 2/Phase 3+4 — the orchestrator owns phase transitions. After approval, announce "Design approved. Handing back to orchestrator for Phase 2."
...
Wait for the user's response. If they request changes, make them and re-run the spec review loop. Only proceed once the user approves. Then hand back to the orchestrator (see scope note above).
